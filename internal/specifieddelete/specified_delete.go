...
func IsSpecifiedDelete(obj metav1.Object) bool {
	_, ok := obj.GetLabels()[batchv1.SpecifiedDeleteKey]
	return ok
}

func PatchPodSpecifiedDelete(c client.Client, pod *corev1.Pod, value string) (bool, error) {
	if _, ok := pod.Labels[batchv1.SpecifiedDeleteKey]; ok {
		return false, nil
	}

	body := fmt.Sprintf(
		`{"metadata":{"labels":{"%s":"%s"}}}`,
		batchv1.SpecifiedDeleteKey,
		value,
	)
